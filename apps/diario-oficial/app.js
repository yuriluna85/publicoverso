/**
 * Publicoverso - apps/diario-oficial/app.js
 * Controlador da Aplicação do Monitor do Diário Oficial (DOU / DOE)
...
(function () {
  ...
  async function inicializar() {
    ...
    try {
      // Carrega o índice de meses e os atos recentes no caminho relativo ../../data/diario_oficial/
  ...
  async function carregarCsvHistorico(caminhoCsv) {
    try {
      const pathRelativo = '../../' + caminhoCsv.replace(/^\.\.\/\.\.\//, '').replace(/^\//, '');
      const resp = await fetch(pathRelativo);
  ...
  function configurarTema() {
    ...
    if (bHighContrast) {
      bHighContrast.addEventListener('click', () => {
        const atual = document.documentElement.getAttribute('data-theme') || 'dark';
        aplicarTema(atual === 'contrast' ? 'dark' : 'contrast');
      });
    }
  ...
  function aplicarTema(tema) {
    ...
    if (btnToggle) {
      btnToggle.textContent = tema === 'dark' ? 'Tema: Claro' : 'Tema: Escuro';
    }
  }
